...
def load_bank_csv(path_or_buf):
    ...
    for i, h in enumerate(header):
        ...
        if h == "date":
            cols.append("Date")
        elif h in ("description", "details", "memo", "narrative", "payee", "merchant", "name"):
...
months_selected = st.sidebar.multiselect(
    "Month(s) (YYYY-MM)", options=months_avail, default=[]
)
weeks_selected = st.sidebar.multiselect(
    "Week(s) (ISO, e.g., 2025-W45)", options=weeks_avail, default=[]
)
days_selected = st.sidebar.multiselect(
    "Day(s) (YYYY-MM-DD)", options=days_avail, default=[]
)
...
amt_lo, amt_hi = st.sidebar.slider(
    "Amount range",
...
df_f = df.copy()
if acc_pick != "All":
    df_f = df_f[df_f["account"] == acc_pick]
if cat_pick != "All":
    df_f = df_f[df_f["category"] == cat_pick]
if year_single != "All":
    df_f = df_f[df_f["y"] == int(year_single)]
...
if days_selected:
    df_f = df_f[df_f["d"].isin(set(days_selected))]

if isinstance(date_range, (list, tuple)) and len(date_range) == 2 and all(
    pd.notna(date_range)
):
...
if io_type == "Expenses (outflow)":
    df_f = df_f[df_f["sign"] == "outflow"]
elif io_type == "Income (inflow)":
    df_f = df_f[df_f["sign"] == "inflow"]
if ignore_small:
    df_f = df_f[df_f["amount"].abs() >= 5]
...
if page == "Dashboard":
    ...
    with c1:
        st.metric("Total Inflow", f"${total_in:,.2f}")
    with c2:
        st.metric("Total Outflow", f"${total_out:,.2f}")
    with c3:
        st.metric("Net", f"${net:,.2f}")
    with c4:
        st.metric("Transactions", f"{len(df_f):,}")
    ...
    st.plotly_chart(
        px.bar(
            top_n_series(spend_by_cat).sort_values(),
            orientation="h",
...
# -----------------------------
    ...
    st.plotly_chart(
        px.line(dly, x="date", y="amount", title="Daily Net Flow"),
        use_container_width=True,
    )
    mon = df_f.groupby("m")["amount"].sum().reset_index()
    st.plotly_chart(
        px.bar(mon, x="m", y="amount", title="Monthly Net Flow", labels={"m": "Month"}),
        use_container_width=True,
    )
...
# -----------------------------
    ...
    st.plotly_chart(
        px.bar(
            top_n_series(spend).sort_values(),
            orientation="h",
...
# -----------------------------
    ...
    tbl = pd.DataFrame(
        {
            ...
            "total": merch.values,
            "transactions": df_f.groupby("desc_norm")["amount"]
            .size()
            .reindex(merch.index)
            .values,
        }
    ...
    st.plotly_chart(
        px.bar(
            top_n_series(top_spend_merch).sort_values(),
            orientation="h",
            title="Top Spending Merchants",
            labels={"value": "Amount", "index": "Merchant"},
        ),
        use_container_width=True,
    )
    ...
    if len(rec) == 0:
        ...
        st.plotly_chart(
            px.line(
                ...
                x="m",
                y="amount",
                title=f"Monthly spend: {rec.iloc[0]['merchant']}",
            ),
            use_container_width=True,
        )
...
# -----------------------------
    ...
    if len(dups) == 0:
        ...
        st.dataframe(
            dups[["date", "description", "amount", "account", "category"]],
            use_container_width=True,
            height=300,
        )
    ...
    if len(anomalies) == 0:
        ...
        show = anomalies[["date", "description", "amount", "score"]].sort_values(
            "score"
        )
...
# -----------------------------
    ...
    years_multi = st.sidebar.multiselect(
        "Compare years (up to 4)", years_avail, max_selections=4
    )
    if not years_multi:
        ...
        st.plotly_chart(
            px.bar(
                ...
                color="y",
                barmode="group",
                title="Yearly spend by category",
            ),
            use_container_width=True,
        )
...
# -----------------------------
    ...
    pareto_tbl = pd.DataFrame(
        {"category": spend_cat.index, "spend": spend_cat.values, "cum_share": pareto.values}
    )
    st.markdown("**Pareto categories (cover ≈80% of expenses):**")
    st.dataframe(
        pareto_tbl[pareto_tbl["cum_share"] <= 0.8],
        use_container_width=True,
        height=260,
    )
    ...
    if len(df) > 0:
        ...
        latest = (
            df[(df["amount"] < 0) & (df["m"] == last_month)]
            .groupby("category")["amount"]
            .sum()
            .abs()
        )
        hot = (
            pd.DataFrame({"baseline_3m_avg": base / 3})
            .join(latest.rename("latest"), how="outer")
            .fillna(0.0)
        )
    ...
    if len(fees):
        ...
        st.success("No obvious fee keywords found.")
...
st.markdown("---")
st.caption(
    "Tips: refine Category Rules • Use AI for uncategorized or confident overrides • "
    "Advanced Filters tame noise • Smart Advisor shows savings ideas"
)
